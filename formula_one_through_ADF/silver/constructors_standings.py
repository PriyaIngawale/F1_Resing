...
results_df = spark.read.load('/mnt/sagen2databricks/f1-project/silver/results')
results_df = results_df.select('race_id','driver_id','constructor_id','points','position','position_text')

# COMMAND ----------

races_df = spark.read.load('/mnt/sagen2databricks/f1-project/silver/race')
...
total_points.write.option('format','delta').mode('overwrite').save('/mnt/sagen2databricks/f1-project/gold/constructors_standings')

# COMMAND ----------

constructors_stand_df = spark.read.load('/mnt/sagen2databricks/f1-project/gold/constructors_standings')


# COMMAND ----------

constructors_df = spark.read.load('/mnt/sagen2databricks/f1-project/silver/constructors').select('constructor_id',col('name').alias('Team'))
